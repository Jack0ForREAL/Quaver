...
namespace Quaver.Shared.Screens.Selection
{
    public sealed class SelectionScreen : QuaverScreen, IHasLeftPanel
    {
        ...
        private void HandleInput(GameTime gameTime)
        {
            ...
            HandleKeyPressF3();
            HandleKeyPressF4();
            HandleKeyPressF5();         // Regular Refresh (F5)
            HandleSmartRefresh();       // NEW: Smart Refresh (Ctrl + F5)
            HandleKeyPressEnter();
        ...
        // STANDARD F5 (Full Refresh) - Now ignores if CTRL is held
        ...
        // --- NEW KEYBIND (Ctrl + F5): QUICK IMPORT ---
        private void HandleSmartRefresh()
        {
            if (!KeyboardManager.IsCtrlDown() || !KeyboardManager.IsUniqueKeyPress(Keys.F5)) return;
            
            // Log to confirm key press was detected
            Logger.Log("[SmartRefresh] Keybind detected! Starting scan...", LogLevel.Important, LogType.Runtime);
            NotificationManager.Show(NotificationLevel.Info, "Scanning for new maps...");
            
            ThreadScheduler.Run(() =>
            {
                var newMaps = MapManager.DetectNewMapsets();
                Logger.Log($"[SmartRefresh] Scan finished. New folders found: {newMaps.Count}", LogLevel.Important, LogType.Runtime);
