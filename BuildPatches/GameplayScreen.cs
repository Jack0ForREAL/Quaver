...
namespace Quaver.Shared.Screens.Gameplay
{
    public class GameplayScreen : QuaverScreen
    {
        ...
        { 
            if (ModeHelper.IsKeyMode(Map.Mode)) Ruleset = new GameplayRulesetKeys(this, Map); 
        ...
        private void HandleFailure()
        {
            ...
            { 
                ...
                { 
                    if (HasQuit && AudioEngine.Track.IsPlaying) AudioEngine.Track.Fade(0, FailFadeTime); 
                    else 
                    { 
        ...
        public void SkipToNextObject(bool force = false)
        {
            ...
            try { AudioEngine.Track?.Seek(skipTime); Timing.Time = AudioEngine.Track.Time; } 
            catch (Exception e) 
            { 
        ...
        public void SetRichPresence()
        {
            ...
            { 
                ...
                { 
                    ...
                    if (view?.ScoreboardLeft != null) alivePlayers = view.ScoreboardLeft.Users.FindAll(x => !x.Processor.MultiplayerProcessor.IsBattleRoyaleEliminated).Count; 
        ...
        public void HandleSuggestedOffsetCalculations()
        {
            ...
            if (stdDev < 25 && !Exiting) DialogManager.Show(new OffsetConfirmDialog(this, (int)samples.Average() + ConfigManager.GlobalAudioOffset.Value)); 
            else ExitOffsetCalibrationOnFailure();
        }
        ...
        {
            try { AudioTrack.AllowPlayback = true; AudioEngine.Track?.Seek(targetSyncTime); Timing.Time = AudioEngine.Track.Time; } 
            catch (Exception e) { Timing.Time = targetSyncTime; }
            finally 
            { 
        ...
        public void SendReplayFramesToServer(bool force = false, bool appendFinishSong = false)
        {
            ...
            { 
                ...
                for (var i = LastReplayFrameIndexSentToServer + 1; i < ReplayCapturer.Replay.Frames.Count; i++) frames.Add(ReplayCapturer.Replay.Frames[i]); 
