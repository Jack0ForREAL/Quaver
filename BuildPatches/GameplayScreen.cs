...
namespace Quaver.Shared.Screens.Gameplay
{
    public class GameplayScreen : QuaverScreen
    {
        ...
        public bool EligibleToSkip
        {
            get
            {
                if (Map.HitObjects.Count == 0) return false;
        ...
        protected virtual void HandleInput(GameTime gameTime)
        {
            ...
            { 
                if (KeyboardManager.IsUniqueKeyPress(ConfigManager.KeyQuickExit.Value)) HandleQuickExit(); 
            }
            ...
            if (!IsPlayComplete && !IsCalibratingOffset)
            {
                ...
                { 
                    if (AudioEngine.Track.IsPlaying) AudioEngine.Track.Pause(); 
                    if (IsTestPlayingInNewEditor) ExitToNewEditor(true); 
        ...
        private void HandlePauseInput(GameTime gameTime)
        {
            ...
            { 
                if (IsPlayTesting) 
                { 
                    if (AudioEngine.Track.IsPlaying) { AudioEngine.Track.Pause(); AudioEngine.Track.Seek(PlayTestAudioTime); } 
                    CustomAudioSampleCache.StopAll(); 
                    if (IsTestPlayingInNewEditor) ExitToNewEditor(); 
                    else Exit(() => new EditorScreen(OriginalEditorMap)); 
                } 
                else if (IsCalibratingOffset) { OffsetConfirmDialog.Exit(this); } 
                else { TimePauseKeyHeld = 0; Pause(gameTime); } 
                return; 
            }
        ...
        public void Pause(GameTime gameTime = null, bool removeMods = true)
        {
            ...
            if (!IsPaused || SpectatorClient != null)
            {
                ...
                { 
                    if (ConfigManager.DisplayPauseWarning.Value) NotificationManager.Show(NotificationLevel.Warning, "WARNING! Your score will not be submitted due to pausing during gameplay!", null, true); 
                    ModManager.AddMod(ModIdentifier.Paused); ReplayCapturer.Replay.Mods |= ModIdentifier.Paused; Ruleset.ScoreProcessor.Mods |= ModIdentifier.Paused; 
                }
                ...
                DiscordHelper.Presence.EndTimestamp = 0; DiscordHelper.UpdatePresence();
                ...
                Utils.NativeUtils.EnableWindowsKey(); return;
            }
            ...
            screenView.PauseScreen?.Deactivate(); SetRichPresence(); 
            OnlineManager.Client?.UpdateClientStatus(GetClientStatus()); 
        ...
        private void HandlePlayRestart(double dt)
        {
            ...
            if (!screenView.FadingOnRestartKeyRelease && screenView.FadingOnRestartKeyPress) { screenView.FadingOnRestartKeyPress = false; screenView.FadingOnRestartKeyRelease = true; screenView.Transitioner.Animations.Clear(); screenView.Transitioner.Animations.Add(new Animation(AnimationProperty.Alpha, Easing.Linear, 1, 0, 200)); }
        }
